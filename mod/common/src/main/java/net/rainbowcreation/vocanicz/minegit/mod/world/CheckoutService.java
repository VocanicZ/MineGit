...
/**
 * Orchestrates a {@code /mg checkout <ref> [--force]} across the mod's two execution contexts (Spec D
 * §5, issue #63) — the loader-agnostic analogue of the plugin's checkout service.
 *
 * <p>The level can only be read <em>and written</em> on the server <strong>thread</strong>, but the
 * dirty-guard + {@code HEAD → ref} diff is git work that must not stall the tick. So a checkout is a
 * five-step thread dance:
 *
 * <ol>
 *   <li><strong>Snapshot (server thread, throttled):</strong> the loaded chunk set is read in batches
 *       of {@code chunksPerTick} into a {@link SnapshotWorldAdapter}, so the dirty-guard can compare
 *       the live world against {@code HEAD} off-tick.
 *   <li><strong>Plan (background):</strong> {@link MineGitRepo#planCheckout} runs the dirty-guard
 *       (unless {@code force}) and computes the {@code HEAD → ref} {@link WorldDiff} — no Minecraft
 *       access, safe off-tick. A dirty tree (without force) or an unresolvable ref produces a {@link
 *       Result} error.
 *   <li><strong>Apply (server thread, throttled):</strong> the diff's {@link ChunkDiff}s are replayed
 ...
 *   <li><strong>Finish (background):</strong> {@link MineGitRepo#finishCheckout} hard-resets the local
 *       ref and {@code .mgc} working tree to the target.
 *   <li><strong>Completion (server thread):</strong> the {@link Result} is delivered to {@code
 *       onComplete} on the server thread, where messaging the player is safe.
 * </ol>
 *
...
public final class CheckoutService {
    ...
    /**
     * @param serverThread runs world reads/applies + the completion callback on the server thread
     * @param background runs the dirty-guard, diff, and ref move (git) off-tick
     * @param chunksPerTick how many chunks to read/apply per server-thread pass; must be {@code >= 1}
     */
    public CheckoutService(Executor serverThread, Executor background, int chunksPerTick) {
    ...
    private void plan(
        ...
        serverThread.execute(() -> applyBatch(units, 0, diff, repoPath, live, clock, target, onComplete));
    }
    ...
    // ---- 3. apply (server thread, throttled) --------------------------------------------------
    ...
    private void applyBatch(
        ...
        background.execute(() -> finish(diff, repoPath, live, clock, target, onComplete));
    }

    // ---- 4. finish (background: move the ref) -------------------------------------------------
