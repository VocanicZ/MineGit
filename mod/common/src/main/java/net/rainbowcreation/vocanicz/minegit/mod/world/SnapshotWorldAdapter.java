...
/**
 * An immutable, read-only {@link WorldAdapter} backed by chunks already captured from the live level
 * (Spec D §5). The mod analogue of the plugin's snapshot adapter, in core-only types.
 *
 * <p>The mod reads loaded chunks on the <strong>server thread</strong> (throttled across ticks),
 * captures them here, then hands this snapshot to {@link net.rainbowcreation.vocanicz.minegit.core.git.MineGitRepo#commit} on
 * a <strong>background</strong> thread. Because every read is served from the captured map — never
 * from the {@code ServerLevel} — the heavy {@code serialize + git} work runs off-tick without ever
 * touching the level from the wrong thread. {@link #allChunks()} and {@link #drainDirty()} both
 * return the captured ref set, matching {@link ModWorldAdapter}'s loaded-chunks behaviour whether
 * core treats this as the first content commit or a later one.
 *
...
public final class SnapshotWorldAdapter implements WorldAdapter {
    ...
    /**
     * @param dimensions the live level's dimensions at capture time
